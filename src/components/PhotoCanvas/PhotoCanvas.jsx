...
class PhotoCanvas extends PureComponent {
  ...
  computeMaskStyle = memoize((
    ...
    if (border === BorderTypes.BORDER) {
      _.set(maskStyle, 'borderWidth', _.round(canvasSize * BORDER_THICKNESS_FACTOR));
    }
  ...
  computeDateStyle = memoize((
    ...
    if (isTrimmingPortrait) {
      _.set(dateStyle, 'right', numeral(((1 - trimmingRatio) / 2) + DATE_MARGIN_PERC).format('0[.]00%'));
      _.set(dateStyle, 'bottom', numeral(((1 - trimmingLongerRatio) / 2) + DATE_MARGIN_PERC).format('0[.]00%'));
    } else {
      _.set(dateStyle, 'right', numeral(((1 - trimmingLongerRatio) / 2) + DATE_MARGIN_PERC).format('0[.]00%'));
      _.set(dateStyle, 'bottom', numeral(((1 - trimmingRatio) / 2) + DATE_MARGIN_PERC).format('0[.]00%'));
    }

    if ((rotate === 90 || rotate === 270) && trimming === TrimmingTypes.IMAGE_FULL) {
      const _longerRatio = Math.max(originalOptionRatio, originalRatio) * originalRatio;
      if (isTrimmingPortrait) {
        _.set(dateStyle, 'right', numeral(((1 - originalOptionRatio) / 2) + DATE_MARGIN_PERC).format('0[.]00%'));
        _.set(dateStyle, 'bottom', numeral(((1 - _longerRatio) / 2) + DATE_MARGIN_PERC).format('0[.]00%'));
      } else {
        _.set(dateStyle, 'right', numeral(((1 - _longerRatio) / 2) + DATE_MARGIN_PERC).format('0[.]00%'));
        _.set(dateStyle, 'bottom', numeral(((1 - originalOptionRatio) / 2) + DATE_MARGIN_PERC).format('0[.]00%'));
      }
    }
  ...
  getTrimmingRatios = memoize((originalRatio, originalOptionRatio, originalLongerRatio, trimming) => {
    const trimmingRatio = (trimming === TrimmingTypes.PAPER_FULL)
      ? originalOptionRatio
      : Math.min(originalOptionRatio, originalRatio);
    const trimmingLongerRatio = (trimming === TrimmingTypes.PAPER_FULL)
      ? 1.0
      : Math.min(trimmingRatio * originalLongerRatio, 1.0);
    const isTrimmingOverRatio = originalRatio / originalOptionRatio >= 1;

    return {
      trimmingRatio, trimmingLongerRatio, isTrimmingOverRatio,
    }
  });
  ...
  computeImageStyle = memoize((
    ...
    if (isTrimmingPortrait) {
      ...
      _.set(imageStyle, 'height', numeral(trimmingLongerRatio).format('0[.]00%'));
    } else {
      _.set(imageStyle, 'width', numeral(trimmingLongerRatio).format('0[.]00%'));
    ...
    if (trimming === TrimmingTypes.IMAGE_FULL) {
      if (border === BorderTypes.BORDER) {// 2022.02.22 이미지풀 유테 잘리는면 표현 보정
        ...
        if (isTrimmingPortrait) {
          ...
          _.set(imageStyle, 'width', numeral(trimmingLongerRatio-0.05).format('0[.]00%'));
          _.set(imageStyle, 'height', numeral(trimmingRatio-0.05).format('0[.]00%'));
        }    
      }
      ...
      if (isTrimmingOverRatio) {
        const longerSize = numeral((originalRatio / originalOptionRatio) * scale).format('0[.]00%');
        ...
        const longerSize = numeral((originalOptionRatio / originalRatio) * scale).format('0[.]00%');
        const stdSize = numeral(1 * scale).format('0%');
        if (isPortrait) {
          _.set(imageStyle, 'backgroundSize', `${stdSize} ${longerSize}`);
        } else {
    ...
    if (rotate === 90 || rotate === 270) {
      if (trimming === TrimmingTypes.IMAGE_FULL) {
        if (isTrimmingPortrait) {
          const ratioW = Math.max(originalOptionRatio, originalRatio) * originalRatio;
          _.set(imageStyle, 'width', numeral(ratioW).format('0[.]00%'));
          _.set(imageStyle, 'height', numeral(originalOptionRatio).format('0[.]00%'));
        } else {
          const ratioH = Math.max(originalOptionRatio, originalRatio) * originalOptionRatio;
          _.set(imageStyle, 'width', numeral(originalOptionRatio).format('0[.]00%'));
          _.set(imageStyle, 'height', numeral(ratioH).format('0[.]00%'));
        }
        _.set(imageStyle, 'backgroundPosition', 'center');
      } else {
        if (isTrimmingPortrait) {
          const longerSize = numeral((originalLongerRatio / originalOptionRatio) * scale).format('0[.]00%');
          const stdSize = numeral(1 * scale).format('0%');
        ...
        if (isRelativePositions) {
          _.set(imageStyle, 'backgroundPositionX', numeral(posY + 0.5).format('0[.]00%'));
          _.set(imageStyle, 'backgroundPositionY', numeral(posX + 0.5).format('0[.]00%'));
        } else {
          _.set(imageStyle, 'backgroundPositionX', `calc(50% ${formatPos(posY)})`);
          _.set(imageStyle, 'backgroundPositionY', `calc(50% ${formatPos(posX)})`);
        }
      }
    } else {
      if (trimming === TrimmingTypes.PAPER_FULL) {
        if (isRelativePositions) {
          _.set(imageStyle, 'backgroundPositionX', numeral(posX + 0.5).format('0[.]00%'));
          _.set(imageStyle, 'backgroundPositionY', numeral(posY + 0.5).format('0[.]00%'));
        } else {
          _.set(imageStyle, 'backgroundPositionX', `calc(50% ${formatPos(posX)})`);
          _.set(imageStyle, 'backgroundPositionY', `calc(50% ${formatPos(posY)})`);
        }
      }
    }
  ...
  render() {
    ...
    const {
      trimmingRatio, trimmingLongerRatio, isTrimmingOverRatio,
    } = this.getTrimmingRatios(originalRatio, originalOptionRatio, originalLongerRatio, trimming);

    const imageStyle = this.computeImageStyle(
      border,
      isPortrait, isTrimmingPortrait, isRelativePositions,
      src, trimming, rotate, posX, posY, scale,
      originalRatio, originalOptionRatio, originalLongerRatio,
      trimmingRatio, trimmingLongerRatio, isTrimmingOverRatio,
    );

    const trimmingStyle = this.computeTrimmingStyle(originalOptionRatio, isTrimmingPortrait);
    ...
    const dateStyle = this.computeDateStyle(
      originalRatio, originalOptionRatio, trimmingRatio, trimmingLongerRatio,
      isTrimmingPortrait, rotate, trimming,
    );
    ...
    return (
      <div
        className={classNames(styles.PhotoCanvas, className, _.get(styles, zoomClassName))}
      >
        <div
